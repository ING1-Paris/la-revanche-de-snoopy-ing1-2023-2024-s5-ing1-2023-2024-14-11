...
void handleKeypress(struct ModeleNiveau *modele, int *temps_arrivee, int *pause, int *menu_principal, char *derniere_direction) {
    if (kbhit()) {
        ...
        switch (keyPressed) {
            case 'p': handlePause(modele, temps_arrivee, pause); break;
            case 's': handleSauvegarder(modele, temps_arrivee, pause, menu_principal); break;
            case 'q': handleAbandonner(pause, menu_principal); break;
            case 'r': handleRecommencer(modele, temps_arrivee, pause, menu_principal, derniere_direction); break;
            case 32: handleEspace(modele, pause, derniere_direction); break;
            case 224: handleArrowKeys(modele, pause, derniere_direction); break;
        }
    }
}

void handlePause(struct ModeleNiveau *modele, int *temps_arrivee, int *pause) {
    if (*pause == 0) {
        *pause = (int) time(NULL);
        wprintf(L"Pause\n");
    } else {
...
void handleSauvegarder(struct ModeleNiveau *modele, int *temps_arrivee, int *pause, int *menu_principal) {
    if (*pause == 0) {
...
void handleRecommencer(struct ModeleNiveau *modele, int *temps_arrivee, int *pause, int *menu_principal, char *derniere_direction) {
    if (*pause != 0) return;
    if (modele->vies_restantes <= 1) {
        system("cls");
        wprintf(L"Vous n'avez plus qu'une vie restante ! Vous ne pouvez donc pas recommencer\n");
        sleep(1);
        return;
    }
    *derniere_direction = ' ';
    ...
    *temps_arrivee = nouveau_timer();
    ...
    system("cls");
    afficherFichier("../assets/ASCII/recommence.txt");
    sleep(1);
}
